...
namespace Revive_Ui
{
	public partial class Form3 : MetroForm
	{
					...
					try
					{
						...
						if (this.cboxMaterials.SelectedIndex >= 0){
							...
							if (str.Length > 1)
							{
								...
								string refrence = RefGen.GetUniqueKey(90);
								...
								string statement2 = "insert into transactionlog (company, truck, weightBefore, weightAfter, material, amountDue, transactionTime, paymentType, transCode) values('" + text1 + "','" + text4 + "', '" + (object)num1 + "', '" + (object)num2 + "', '" + text3 + "', '" + (object)num4 + "', '" + (object)totalSeconds + "', '" + str + "', '"+ refrence + "')";
								Revive_Ui.model.model model = new Revive_Ui.model.model();
								model.execQuery(statement1);
		...
		{
			if (txtVehicleNo.Text.Length > 1)
			{
				string Q = "select*from checks where truckReg ='"+txtVehicleNo.Text+"'";
				try
				{
					model.model mod = new model.model();
					if (mod.colExists(Q) > 0){
						Q = "delete from checks where truckReg = '" + txtVehicleNo.Text + "'";
						mod.execQuery(Q);
						MessageBox.Show("the transaction has been cancelled", "Transaction Alert", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
					} else {
						MessageBox.Show("the transaction Reference not found", "Transaction Alert", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
					}
				}
				catch(Exception)
				{
		...
		private void metroButton1_Click(object sender, EventArgs e){
			model.model mod = new model.model();
			var datx = mod.writeTransactions("select*from transactionlog");
			//Console.WriteLine(datx.mObj);
			var URL = Revive_Ui.Properties.Resources.StagingBaseUrl.ToString() + "transaction/sync";
		}
	}
}
